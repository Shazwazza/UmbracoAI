<#
.SYNOPSIS
    Starts ONE shared, headed Playwright MCP server over HTTP for the Umbraco demo.

.DESCRIPTION
    Both the main Copilot CLI session (.mcp.json) and the Conductor workflow
    (umbraco-demo.yaml) connect to this single server over HTTP. Because the
    server is started with --shared-browser-context, every connected client
    drives the SAME browser window - so the audience always sees the live
    interactions instead of a hidden, behind-the-scenes browser.
